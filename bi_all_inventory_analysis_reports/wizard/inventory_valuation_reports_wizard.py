...
class inventory_valuation_report_wizard(models.TransientModel):
    ...
    @api.onchange('start_date', 'end_date', 'warehouse', 'category', 'location_id', 'company_id', 'display_sum', 'filter_by', 'product_ids')
    ...
    def print_report(self):
        ...
        return self.env.ref('bi_all_inventory_analysis_reports.inventory_product_category_template_pdf').report_action(self)
    ...
    def _compute_quantities_product_quant_dic(self, lot_id, owner_id, package_id, from_date, to_date, product_obj, data):
        ...
        domain_move_in = [('product_id', 'in', product_obj.ids),
                          ('location_dest_id', 'in', locations)] + domain_move_in_loc
        domain_move_out = [('product_id', 'in', product_obj.ids)
                           ] + domain_move_out_loc + custom_domain
        ...
        quants_res = dict((item['product_id'][0], item['quantity']) for item in Quant.read_group(
    ...
    def get_lines(self, data):
        ...
        for product in product_res:
            ...
            stock_internal_lines = self.env['stock.move'].search([('location_id.usage', '=', 'internal'), (
            ...
            ending_bal = opening[product.id]['qty_available'] - \
                sales_value + incoming + adjust
            method = ''
        ...
        if xlwt:
            ...
            if self.display_sum:
                worksheet.write_merge(
                    0, 1, 1, 6, "Inventory Valuation Summary Report", style=style_title)
                ...
                worksheet.write_merge(
                    0, 1, 2, 9, "Inventory Valuation Report", style=style_title)
            ...
            export_id = self.env['inventory.valuation.report.report.excel'].create(
                {'excel_file': base64.encodebytes(fp.getvalue()), 'file_name': filename})
            res = {
                'view_mode': 'form',
                'res_id': export_id.id,
                'res_model': 'inventory.valuation.report.report.excel',
...
class inventory_valuation_report_report_excel(models.TransientModel):
    _name = "inventory.valuation.report.report.excel"
    _description = "Inventory Valuation Report Excel"
