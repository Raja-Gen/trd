...
class ProductRecommendation(models.TransientModel):
    ...
    def days_change(self):
        if self.analysis_days:
            self.past_sale_date = (date.today(
            ) - timedelta(days=self.analysis_days)).strftime(DEFAULT_SERVER_DATE_FORMAT)

    @api.model
    def default_get_past_date(self):
        res_obj = self.env['res.config.settings']
        res_ids = res_obj.search([], limit=1)
        diff = (date.today() - timedelta(days=res_ids.past_sale)
                ).strftime(DEFAULT_SERVER_DATE_FORMAT)
        return diff

    analysis_days = fields.Integer(
        string='Analysis in Days', default=default_get_analysis_days, required=True)
    partners = fields.Many2one(
        'res.partner', string='Choose Vendor', required=True)
    past_sale_date = fields.Date(
        "Past Sales Start From", default=default_get_past_date)
    ...
    def open_table(self):
        ...
        order_line_obj = self.env['sale.order.line'].search([('product_id', 'in', product_list), ('order_partner_id', '=', self.partners.id), (
            'order_id.date_order', '>=', self.past_sale_date), ('order_id.date_order', '<=', str(date.today()))])
        ...
        order_line_obj = self.env['sale.order.line'].search([('order_partner_id', 'in', ware_part_list), (
            'order_id.date_order', '>=', self.past_sale_date), ('order_id.date_order', '<=', str(date.today()))])
        for line in order_line_obj:
            new_record.create({
                'product_id': line.product_id.id,
        ...
        tree_view_id = self.env.ref(
            'bi_all_inventory_analysis_reports.view_tree_get_product_recommendation').id
    ...
    def print_exl_report(self):
        ...
        worksheet.write_merge(
            0, 1, 0, 5, "Product Recommendation Report", style=style_title)
        ...
        quant_obj = self.env['stock.quant'].search([('location_id', 'in', ware_list), (
            'product_id', 'in', product_list), ('in_date', '>=', self.past_sale_date), ('in_date', '<=', current_date)])
        if quant_obj:
            row = 8
            col = 0
            count = 1
            for quant in quant_obj:
                ...
                worksheet.write(
                    row, col+1, quant.product_id.default_code, style)
                worksheet.write(row, col+2, quant.product_id.name, style)
                worksheet.write(
                    row, col+3, quant.location_id.company_id.partner_id.name, style)
                worksheet.write(row, col+4, quant.quantity, style)
                worksheet.write(
                    row, col+5, quant.product_id.sales_count, style)
                row += 1
                count += 1
        fp = io.BytesIO()
        workbook.save(fp)

        export_id = self.env['cover.report.excel'].create(
