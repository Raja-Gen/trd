...
class Inventory_ABC_analysis_wizard(models.Model):
    ...
    @api.onchange('type', 'from_date', 'to_date', 'company_ids', 'category_ids', 'product_ids')
    ...
    def print_inventory_ABC_report(self):
        ...
        fp = io.BytesIO()
        workbook.save(fp)
        abc_id = self.env['inventory.abc.extended'].create({'excel_file': base64.encodebytes(fp.getvalue()), 'file_name': filename})
        fp.close()
    ...
    def tree_graph_report_view(self):
        ...
        display = []
        graph_id = self.env.ref('bi_all_inventory_analysis_reports.inventory_abc_extended_report_graph').id
        tree_id = self.env.ref('bi_all_inventory_analysis_reports.inventory_abc_extended_report_tree').id
...
class Inventory_ABC_analysis_Extended(models.TransientModel):
    ...
    company_ids = fields.Many2many("res.company", 'rel_company_abc', 'company_id', 'abc_id', string="Company")
