...
class InventorycoverReport(models.Model):
    ...
    product_type = fields.Selection([('all', 'All'), ('out', 'Out of Stock Products'), (
        'in', 'Available Products')], 'Filter Products', default='all')
    ...
    def get_months(self):
        ...
        for i in range(self.analysis_days):
            ...
            final_dict.setdefault(date_delta.strftime(
                "%B-%Y"), []).append(date_delta.strftime('%d'))
        return final_dict
    ...
    def validate_product_days(self):
        if not self.product_ids or self.analysis_days <= 0:
            raise ValidationError(
                _('Please enter valid Analysis Days and Products !'))
    ...
    def get_product_stock(self):
        ...
        if self.product_type == "all":
            obj = self.env['stock.quant'].search([('product_id', 'in', ware_product_list), (
                'product_id', 'in', product_list), ('in_date', '<=', self.report_date), ('location_id.usage', '=', 'internal')])
            return obj
        if self.product_type == "out":
            obj = self.env['stock.quant'].search([('product_id', 'in', ware_product_list), ('product_id', 'in', product_list), (
                'in_date', '<=', self.report_date), ('location_id.usage', '=', 'internal'), ('quantity', '<=',  0)])
            return obj
        if self.product_type == "in":
            obj = self.env['stock.quant'].search([('product_id', 'in', ware_product_list), ('product_id', 'in', product_list), (
                'in_date', '>=', conv_date_str), ('in_date', '<', self.report_date), ('location_id.usage', '=', 'internal'), ('quantity', '>',  0)])
            return obj
    ...
    def get_report(self):
        ...
        return self.env.ref('bi_all_inventory_analysis_reports.inventory_coverage_report').report_action(self, data=data)
    ...
    def print_exl_report(self):
        ...
        worksheet.write_merge(
            0, 1, 0, 17, "Inventory Coverage Report", style=style_title)
