...
class stock_data(models.Model):
    ...
    _description = 'Description for stock_data'

class StockRotationReport(models.TransientModel):
    _name = 'stock.rotation.report'
    _description = 'Description for StockRotationReport'
    ...
    def generate_xls_report(self):
        ...
        fp = io.BytesIO()
        workbook.save(fp)
        export_id = self.env['stock.excel.report'].create({'excel_file': base64.encodebytes(fp.getvalue()), 'file_name': filename})
        res = {'view_mode': 'form',
               'res_id': export_id.id,
...
class stock_excel_report(models.TransientModel):
    ...
    _description = "Description for stock_excel_report"
