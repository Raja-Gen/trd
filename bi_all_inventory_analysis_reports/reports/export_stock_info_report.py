...
class ExportStockInformationReport(models.AbstractModel):
    _name = 'report.bi_all_inventory_analysis_reports.exportstock'
    ...
    def _get_domain_locations_new(self, location_ids, company_id=False, compute_child=True):
        ...
        for location in hierarchical_locations:
            ...
            dest_loc_domain = dest_loc_domain and [
                '|'] + dest_loc_domain or dest_loc_domain
            dest_loc_domain.append(
                ('location_dest_id.parent_path', '=like', location.parent_path + '%'))
        if other_locations:
            loc_domain = loc_domain and ['|'] + loc_domain or loc_domain
            loc_domain = loc_domain + \
                [('location_id', operator, other_locations.ids)]
            dest_loc_domain = dest_loc_domain and [
                '|'] + dest_loc_domain or dest_loc_domain
        ...
        return (
            domain + loc_domain,
            domain + dest_loc_domain +
            ['!'] + loc_domain if loc_domain else domain + dest_loc_domain,
            domain + loc_domain +
            ['!'] + dest_loc_domain if dest_loc_domain else domain + loc_domain
        )
    ...
    def _compute_quantities_dict(self, product_id, lot_id=False, owner_id=False, package_id=False, location_id=False, warehouse_id=False, from_date=False, to_date=False):
        ...
        domain_quant = [('product_id', 'in', [product_id.id])
                        ] + domain_quant_loc
        ...
        domain_move_in = [
            ('product_id', 'in', [product_id.id])] + domain_move_in_loc
        domain_move_out = [
            ('product_id', 'in', [product_id.id])] + domain_move_out_loc
        ...
        if warehouse:
            ...
            for product_id in product_ids:
                ...
                res = self._compute_quantities_dict(product_id,
                                                    lot_id=self._context.get(
                                                        'lot_id'),
                                                    warehouse_id=warehouse,
                                                    from_date=data.get(
                                                        'start_date'),
                                                    to_date=data.get('end_date'))
                qty_available = res.get(product_id.id).get(
                    'qty_available') or 0.0
                incoming_qty = res.get(product_id.id).get(
                    'incoming_qty') or 0.0
                outgoing_qty = res.get(product_id.id).get(
                    'outgoing_qty') or 0.0
                virtual_available = res.get(product_id.id).get(
    ...
    def _get_report_values(self, docids, data=None):
        ...
        date_to = datetime.strptime(
            data['form']['date_to'], "%Y-%m-%d").strftime("%d-%m-%Y")
        date_from = datetime.strptime(
            data['form']['date_from'], "%Y-%m-%d").strftime("%d-%m-%Y")
        data = {
            'end_date': end_date,
