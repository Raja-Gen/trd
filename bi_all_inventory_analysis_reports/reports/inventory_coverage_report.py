...
class ReportInventoryCoverage(models.AbstractModel):
    _name = 'report.bi_all_inventory_analysis_reports.coverage_report'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if product_type == "all":
            quant_obj = self.env['stock.quant'].search([('product_id', 'in', ware_prd_list), (
                'product_id', 'in', product_list), ('in_date', '<=', report_date), ('location_id.usage', '=', 'internal')])
        if product_type == "out":
            quant_obj = self.env['stock.quant'].search([('product_id', 'in', ware_prd_list), ('product_id', 'in', product_list), (
                'in_date', '<=', report_date), ('location_id.usage', '=', 'internal'), ('quantity', '<=',  0)])
        if product_type == "in":
            quant_obj = self.env['stock.quant'].search([('product_id', 'in', ware_prd_list), ('product_id', 'in', product_list), (
                'in_date', '>=', conv_date_str), ('in_date', '<=', report_date), ('location_id.usage', '=', 'internal'), ('quantity', '>',  0)])
        for quant in quant_obj:
            cl_st = ((quant.quantity + quant.product_id.purchased_product_qty) -
                     quant.product_id.sales_count)
            docs.append({
