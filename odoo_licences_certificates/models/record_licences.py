...
class Licences(models.Model):
    _name = "record.licences"
    _description = "Licences History"
    ...
    def unlink(self):
        for rec in self:
            if rec.state not in ('a_draft', 'cancel'):
                raise UserError(_('You cannot delete Licence.'))
        return super(Licences, self).unlink()

    @api.model
    def _run_licences_certificate_cron_new(self):
        licences_certificate = self.env['record.licences'].search([])
        for rec in licences_certificate:
            ...
            if expire_date == fields.date.today():
                ...
                # mail_template_id = self.env.ref('odoo_licences_certificates.odoo_licences_record_email_templte_history')
                mail_template_id = self.env.ref('odoo_licences_certificates.odoo_licences_record_email_templte_history_view')
            ...
            if rec.end_date == fields.date.today():
                ...
                # mail_template_id = self.env.ref('odoo_licences_certificates.odoo_licences_template_history_record')
                mail_template_id = self.env.ref('odoo_licences_certificates.odoo_licences_template_history_record_view')
                ctx = self._context.copy()
                ctx.update({'name': str(rec.name)  +  rec.end_date.strftime('%m/%d/%y')})
                mail_template_id.with_context(ctx).send_mail(rec.id)
